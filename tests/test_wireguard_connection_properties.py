...
Note: These tests create NM.SimpleConnection objects but do not require
a running NetworkManager daemon or D-Bus connection.

IMPORTANT: These tests currently use mock WireGuard keys for testing the
connection structure. In production, valid Curve25519 keys must be used.
The tests verify the structure and configuration logic, but full validation
requires valid keys generated by `wg genkey` or equivalent.
"""
...
# Skip all tests if NetworkManager is not available
pytestmark = pytest.mark.skipif(
    not hasattr(NM, 'SimpleConnection'),
    reason="NetworkManager GObject introspection not available"
)
